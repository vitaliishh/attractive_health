...
public class Posts1 extends AppCompatActivity implements View.OnClickListener {
    ...
    public void init() {
        ...
        Cursor cursor = database.rawQuery("select * from diet where type = ? and gender = ?", new String[]{"standart", "man"});
//        Cursor cursor = database.rawQuery("select * from diet", null);

        //Cursor cursor = database.query(DBHelperStatic.TABLE_DIET, null, "type = ?", new String[]{"standart"}, null, null, null);

        Cursor st = database.rawQuery("select COUNT(*) from diet where type = ? and gender = ?", new String[]{"standart", "man"});
//        Cursor st = database.rawQuery("select COUNT(*) from diet",null);
