...
public class Posts2 extends AppCompatActivity implements View.OnClickListener {
    ...
    public void init() {
        ...
        Cursor cursor = database.rawQuery("select * from training where type = ? and gender = ?", new String[]{"standart", "man"});
        Cursor st = database.rawQuery("select COUNT(*) from training where type = ? and gender = ?", new String[]{"standart", "man"});
